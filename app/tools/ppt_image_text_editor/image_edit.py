...
def image_format_for_path(media_path: str) -> str:
    return _FORMAT_BY_EXT.get(Path(media_path).suffix.lower(), "PNG")
...
def estimate_background(img: Image.Image, box: tuple[int, int, int, int]) -> tuple[int, int, int]:
    ...
    samples = []
    regions = [(x0-pad,y0-pad,x1+pad,y0),(x0-pad,y1,x1+pad,y1+pad),(x0-pad,y0,x0,y1),(x1,y0,x1+pad,y1)]
...
def _fit_font(draw: ImageDraw.ImageDraw, text: str, target_w: int, target_h: int, font_path: str | None):
    lo, hi = 5, max(6, int(target_h * 1.45)); best = _load_font(font_path, lo)
    while lo <= hi:
        mid = (lo + hi) // 2; font = _load_font(font_path, mid); bbox = draw.textbbox((0, 0), text, font=font)
...
def edit_text(image_bytes: bytes, *, box: tuple[int, int, int, int], new_text: str,
              font_path: str | None = None, pad_px: int = 2, output_format: str = "PNG") -> bytes:
    ...
    bg = estimate_background(img, (x0,y0,x1,y1)); fg = estimate_foreground(img, (x0,y0,x1,y1), bg)
    draw = ImageDraw.Draw(img); erase = _clip_box((x0-pad_px,y0-pad_px,x1+pad_px,y1+pad_px), img.size)
    draw.rectangle(erase, fill=bg)
    if new_text:
        target_w=max(1,erase[2]-erase[0]-2); target_h=max(1,erase[3]-erase[1]-2)
        font=_fit_font(draw,new_text,target_w,target_h,font_path); tb=draw.textbbox((0,0),new_text,font=font)
        tw,th=tb[2]-tb[0],tb[3]-tb[1]; tx=erase[0]+max(1,(target_w-tw)//2); ty=erase[1]+max(1,(target_h-th)//2)-tb[1]
        draw.text((tx,ty),new_text,font=font,fill=fg)
    out=io.BytesIO(); fmt=(output_format or "PNG").upper(); kwargs={"quality":95} if fmt=="JPEG" else {}
    img.save(out,format=fmt,**kwargs); return out.getvalue()
