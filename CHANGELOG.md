**繁體中文** ｜ [English](CHANGELOG_en.md) ｜ [日本語](CHANGELOG_ja.md)
...
### `jtdt-reform` 引擎補上逐頁進度 —— 三顆引擎到此都有了

`pdf-to-office` 的 `pdf2docx-refine` 與 `jtdt-layout` 早就逐頁回報，
**只有 `jtdt-reform` 從頭到尾只有兩個點**（開始、結束）。大檔動輒數分鐘，
中間畫面完全不動 —— 而「什麼都沒發生」正是最難診斷的症狀，使用者會以為當掉。

現在解析與產生文書檔兩個階段都逐頁回報（實測 20 頁的文件回報 41 次，
進度 0.07 → 0.95）。

> **進度是附屬品不是產出** —— 回報函式自己丟例外時，轉檔仍然要成功。
> 守門專門試一次「回報函式爆掉」。
>
> **文件上寫著的狀態也要核實**：我們自己的筆記上寫著「三條引擎都接上了」，
> 去查才發現只有兩顆。寫下來的東西會變成下一個人相信的事實。

## [1.15.58] - 2026-09-16

### ⚠⚠ 文件差異比對：**插一頁，後面每一頁都被判成整頁不同**

頁面配對原本是**依索引硬配**（舊版第 N 頁對新版第 N 頁）。實測 20 頁的文件
在第 3 頁插一頁：**只有 2 / 21 頁對得上，其餘 19 頁全部變成「整頁刪掉 ＋
整頁新增」** —— 使用者看到的是「整份都改了」，而實際上只多了一頁。

改成跟行的比對**同一個結構、只是高一層**：用序列比對把沒改的頁面當錨點
1:1 配起來，改過的頁面夾在錨點之間位置自然對得上。同一份文件插一頁之後，
現在只有那一頁算差異。

> **不需要模糊相似度** —— 沒改的頁面本來就逐字相同，拿它們當錨點就夠了。
> 加相似度只會多一個要調的門檻。

> 兩邊的頁碼在插過頁之後會錯開，所以**標題要講出來**（「舊版第 4 頁 ↔
> 新版第 5 頁」），而且頁面模式抓頁面圖要用**真正的頁碼**不是畫面上第幾列。

### 按「學起來」時會告訴你這個欄位名稱還對應到誰

表單自動填寫的「學起來」寫進的是**全站共用**的欄位名稱對照表。原本按下去
只回一句「已學會」，使用者不知道自己影響了什麼。

> **不是改成「偵測到衝突就拒絕」** —— 去看比對的建索引流程才發現前提是錯的：
> **一個標籤對應到多個欄位是刻意支援的**。台灣的表單常有一格同時管兩件事
> （「發票聯數 & 種類」同時屬於 發票種類 與 稅別），讓兩個欄位各自去勾自己的
> 選項。改成拒絕會讓那類表單的勾選整排失效，**而且不會有任何測試變紅**。
> 所以是把後果講出來，不是擋下來。

## [1.15.57] - 2026-09-16

### 日文文件的去識別化補上運転免許証番号與健康保険証

日文文件的支援本來就有（個人番号 ＋ 檢查碼、法人番号 ＋ 檢查碼、電話、
郵便番号、住所、氏名）。這一版補上原本規劃裡剩下的兩類。

**這兩類只認「標籤 ＋ 值」，遮的也只有值那一段**：

* **運転免許証番号是 12 碼，跟マイナンバー一樣長**，而且公開資料裡沒有可以
  驗的檢查碼演算法。沒有標籤的話，任何 12 碼數字都會在兩類之間二選一，
  **兩邊都會誤判 —— 而畫面會顯示「已處理」**。
* **健康保険証的格式依保險者而異**，沒有全國統一的長度或檢查碼。這一條還
  要求後面**確實跟著「番号」**，不然一般文件裡的「記号：A」也會中。

> **值裡面的連字號要用「各種短橫」的字元類** —— PDF 抽出來的是不斷行連字號
> `U+2011` 不是 `-`（本專案記過同一條）。測試素材直接寫死 `\xa0` 與 `\u2011`，
> 不要用自己打的字元，否則測試會過而實際檔案抓不到。

> 誤判語料照樣是驗收的一部分：一份滿是型號 / 注文番号 / ISBN 的日文文件，
> 敏感類別命中必須是 **0**。**只驗「抓得到」的話，把式子放寬到抓一切也會過。**

### 隱藏內容掃描改在**獨立行程**裡解析（外部稽核 F04）

稽核說的是「PyMuPDF 上游明文不支援多執行緒」。**照字面去加 lock 是解錯問題**
—— 我們沒有共用 `Document` 物件（每件工作自己開自己關），報告自己也說沒有
重現崩潰；而且散在各工具加 lock 既無法確認每個入口都遵守，**也隔離不了
解析器當機**。

真正值得做的是**爆炸半徑**：MuPDF 在 C 層 segfault 會把**整個服務行程**帶走
（所有進行中的作業與正在看網頁的人一起沒了）。隔離之後只有那一次請求失敗。

**先只做「隱藏內容掃描」一支** —— 判準是「輸入最不可信 ＋ 一垮就影響最多人」，
而那支工具的用途就是「這份檔案可能有問題，幫我看一下」。其餘工具照舊，
等這條路在正式機跑過一段時間再擴大。

> **用 `subprocess` 不用 `multiprocessing`**：後者的 spawn 會讓子行程**重新
> import 父行程的 `__main__`**，而我們是 `python -m app.main` 起服務 ——
> 那等於每次隔離呼叫都把整個服務重建一次。

> **⚠ 規劃時量的成本是錯的。** 原本量「spawn ＋ import PyMuPDF ＋ 開檔」＝
> 385 ms，**沒有把我們自己的 import 鏈算進去**。實作後第一次量是 **1.6 秒**
> —— 因為子行程 `import app.tools.…` 會觸發工具套件的 `__init__.py`
> （它匯入 router），把整個 Web 框架與設定鏈拉進來（單那一行就 1,375 ms）。
> 把掃描核心抽成一支**只 import PyMuPDF** 的模組、並改成**照檔案路徑載入**
> 之後，固定成本降到 **約 0.5 秒**。守門盯著那支核心的 import 清單。

> **隔離是防護不是功能**：子行程起不來時要能退回同一個行程做完，
> 不可以因為它壞掉就整支工具不能用。毀損檔案的例外型別也要留住
> （跨行程傳的時候很容易弄丟）—— 不然使用者會從 400「你的檔案壞了」
> 變成看到 500「伺服器壞了」，然後一直重試。

> **驗收兩個方向都跑**：同一段會崩潰的程式碼，不隔離時主行程必須死、
> 隔離時必須活著並回報。只驗「隔離之後還活著」證明不了任何事 ——
> 說不定那個崩潰根本沒發生。另外**真的送一次請求進去**確認端點有走隔離：
> 只測內部函式的話，把端點改回不隔離會全綠（稽核 F06 記過同一個洞）。

## [1.15.56] - 2026-09-16

### 文件差異比對多了「頁面模式」—— 把差異**直接框在原本的頁面上**

原本只有文字模式：左右兩欄的文字列表，看得到「改成什麼」，**看不出那句話在
頁面上的哪裡**。現在標題列多一個切換：

* **文字模式**（原本那個）—— 一行都沒動。
* **頁面模式** —— 左右各一張**原本的頁面圖**，差異直接框在上面。
  紅＝被刪、綠＝新增、黃＝修改。

兩個模式**共用同一次比對的結果**，切換不重新上傳也不重新比對。
Office 檔（Word / Excel / PowerPoint / ODF）一樣可以用 —— 它本來就會先轉成
PDF 再比，所以頁面模式看到的是**轉檔後的版面**。

> **比對本身一個字都沒動。** 行還是從 `get_text("text")` 取，只是另外用
> `rawdict` 拿每個字的座標，靠「第幾個非空行」把兩份對起來。
> 實測 30 份真實樣本 **101 / 101 頁對得起來**；對不起來的頁面**整頁不畫框**
> —— 沒有框只是少一個功能，**框畫錯位置是騙人**。

> **行內的字元範圍本來就算出來了，只是算完就丟掉** —— 留下來才標得出
> 「這一行的第幾個字改了」。中文一定要走字元級：`get_text("words")` 對中文
> 會把**整行當成一個「詞」**回傳（沒有空格），用它標示的話改一個字會框整行。

### ⚠ 轉向過的頁面（`/Rotate`）差點全部框錯位置

文字的座標跟算出來的頁面圖**不在同一個座標系**。實測同一頁設不同旋轉角，
量「框裡有多少墨水」：

| 旋轉 | 直接用座標 | 乘上 `page.rotation_matrix` |
|---:|---:|---:|
| 0 | 22.7% | 22.7% |
| 90 | **0.0%** | 22.7% |
| 180 | **0.0%** | 22.6% |
...
沒有人會發現。所以驗收的判準是**框內的墨水覆蓋率**，不是「有沒有畫出框」：
四種旋轉角各驗一次，變異驗證過（拿掉那一行 → 90 / 180 / 270 三條紅）。

### 抽不到文字時會**講出來**，不會假裝沒有差異

掃描件、文字被轉成外框、文字對應表壞掉的 PDF —— 這幾種抽不到座標，
頁面模式會在那一頁寫明「抽不到文字座標，請切到文字模式看內容」，
並在最上面指路去 OCR。**「沒有框」跟「這一頁沒改」長得一模一樣**，
所以這句話不能省。

### ⚠ 一份設定抄了八次，第八次抄壞了整支測試

無頭瀏覽器測試的「找瀏覽器」與「挑一個瀏覽器讀得到的素材目錄」原本在
**八支**測試裡各寫一份。這次新加的那一份只比對路徑字串、沒有讀檔案內容，
於是判斷不出 snap（Ubuntu 的 `/usr/bin/chromium-browser` 是一支 **shell
包裝腳本**，路徑上看不出任何痕跡）——素材被放到瀏覽器讀不到的地方，
上傳「成功」、畫面上檔名也顯示得出來，只有真的送出時才失敗，
於是整支測試 **skip 掉，而 pytest 輸出裡看起來一切正常**。

收成 `tools/browser_probe.py` 一份，八支全部改用它，並加守門擋第九份。

## [1.15.55] - 2026-09-16

### 工具更名：文件擺正 → **掃描修正**

舊名有兩個問題：**跟「頁面轉向」撞概念**（兩個聽起來都在「把頁面擺正」，但那支
只是整頁旋轉 90 / 180 度），而且「擺正」講不出它其實做**裁邊 ＋ 拉正 ＋
去除不勻底色**三件事。英文 `Document straightening` 與日文 `文書の傾き補正`
同樣只講了拉正那一項 —— **三種語言都偏窄**。

新名跟「掃描拼合」結成同一家族，一眼看得出是同一類事情。
英文 `Scan cleanup`、日文 `スキャン補正`。

> **工具 id 與 API 路徑 `doc-straighten` 一個字都沒動** —— 改 id 要連內建角色、
> 資料庫遷移、既有安裝的權限一起搬，而這裡要改的只是顯示名稱。
> **舊名留在搜尋關鍵字裡**，本來會打「擺正」的人照樣找得到。

### 兩支差異比對工具的「複製全文」按鈕沒有翻譯

`複製舊版全文` / `複製新版全文` 與它們的完成提示，在英文 / 日文介面下都是中文。
成因是那幾顆按鈕寫成 `` `{{ icon(...) }} 複製舊版全文` `` —— **Jinja 會先渲染**，
所以整句沒辦法包 `tr()`，就一直沒人包。修法是把圖示留在 `tr()` 外面。

**只有比對完成、結果出現之後才看得到**，所以逐頁掃描一直掃不到。

### 缺 Office 引擎時「文件差異比對」回 500，應該是 503

500 的意思是「伺服器壞了」—— 使用者會一直重試，監控端也全是假警報。
缺 soffice 是**部署問題**：那台機器少裝東西，訊息要說得出要裝什麼。

本專案早就有 `OfficeUnavailableError` ＋ 一個全域處理器，但這支工具自己
`except Exception` 把它吃掉再包成 500 —— **全域處理器根本看不到那個例外**。

> 守門的判準是「**真的送一份 Office 檔進去、而且讓 soffice 找不到**，看回傳碼」，
> 不是掃原始碼有沒有寫 503。變異驗證過：拿掉修正 → 回 500 → 守門紅。

## [1.15.54] - 2026-09-16

### ⚠ 解除安裝會在開始功能表留下刪不掉的資料夾（實機測 v1.15.53 的安裝程式時抓到）

資料夾名同時是路徑，而登錄檔只記得住**最後一次安裝建的那一個**。
所以「先用語言 A 裝、再用語言 B 裝、然後解除安裝」——**A 的那個資料夾就變成
孤兒**，解除安裝完還留在開始功能表裡，而且從此沒有東西會去刪它。

**這是實機上真的發生的**（`.154`，2026-09-16）：解除安裝之後
`Jason Tools Document Toolbox`（英文名）還在，裡面兩個捷徑都指向已經被刪掉的
安裝目錄。

兩邊都補：

* **解除安裝**：登錄檔那一個刪完之後，**每一種語言的名字都再試一次**
  （加上 v1.15.30 以前寫死的舊中文名），每一個都走同一道
  「必須在 `$SMPROGRAMS\` 底下」的檢查 —— 不是我們的東西不能碰。
* **安裝**：先讀回上一次記下的資料夾，跟這次不同就清掉 ——
  不然換語言重裝之後兩個資料夾會同時掛在那裡，使用者不知道哪一個是活的。

實機驗過：修正前 2 個資料夾 → 安裝修好的版本 → 解除安裝 → **0 個**，
而四個 sqlite 的大小完全沒變（使用者資料保住）。

> **守門走宣告的語言清單**（`tests/test_installer_product_name.py`）：
> 每宣告一種語言就要有對應的 `SM_FOLDER_xx` 而且出現在清理清單裡 ——
> **加第四種語言時這條會先紅**。五個方向都做過變異驗證。

### ⚠⚠ 三元運算只包了**一半** —— 16 處在英文 / 日文介面下顯示中文

包 JS 字串時我們**刻意不包整個三元運算**（包起來的話查表的鍵是執行期才算
出來的，永遠查不到而且無聲），正確寫法是**逐個分支各自包**。問題是自動包
字串的工具遇到三元運算就整條跳過，於是「**只有其中一半被包過**」這種狀態
從來沒有人管。

實際掃出 **16 處**，每一處在英文 / 日文介面下都會冒出中文：

* 管理區的儲存結果只有**失敗**那半翻了，成功那半沒有
* 表單自動填寫、文件去識別化的右鍵選單標題只翻了其中一種情況
* 字數統計**超過一小時**才走到的那條路整串是中文（`3時20分`）
* 字型管理「完成：成功 N 個」後面接的「，失敗 N 個」沒翻
* 文字辨識**排隊超過三秒**才出現的那句沒翻

**這一類逐頁掃抓不到** —— 那些字只在按下去、切換、或數值超過某個門檻之後
才出現。新守門 `tests/test_ternary_branches_go_through_tr.py` 逐檔掃，
四個方向變異驗證過。

> **掃描器不可以用正規式配對引號**：第一版用 `'[^']*'`，遇到
> 「字串 ＋ 樣板字面」混寫的那一行會把兩個不相干的引號配成一對、
> 把中間整段吞進去 —— 一邊誤報一邊漏報。改成逐字元走過，
> 而且**樣板字面要拆開看**（`${…}` 裡面是程式碼不是文字）。
> 改完之後多抓到 3 處真的漏翻，少掉 1 處誤報。

### 公司資料的欄位標題在英文 / 日文介面下整片是中文

表單自動填寫那一頁的公司資料卡有 **50 幾個欄位標題**（公司全名、統一編號、
銀行帳號…）從來沒有譯文。分區標題（公司基本 / 地址 / 聯絡…）更冤枉 ——
**譯文早就在語系檔裡，只是顯示端忘了包 `tr()`**。

顯示端一律改成 `tr(標題)`。**使用者自己改過的欄位名稱不受影響** ——
查不到就原樣顯示，而且管理頁上那個**可編輯**的欄位名稱輸入框刻意維持原樣
（翻掉的話一按儲存就把使用者的欄位名稱換成英文）。

### 示範資料自己抄了一份欄位標題，漂成大陸用語

拍截圖用的示範資料 (`tools/seed_demo_data.py`) 自己寫了一份「欄位代號 →
標題」，跟出貨的那份漂了 **6 個欄位**，其中兩個漂成大陸用語「郵箱」
（台灣要寫「信箱」）—— 而那正是我們自己禁用詞表上的詞，**只是用詞守門
沒有掃到那個資料夾**。示範資料正是要拿去拍截圖公開的。

改成從出貨的那份取（單一來源），用詞守門的範圍也一併補上。

## [1.15.53] - 2026-09-16

### 在**有資料**的實例上掃一次，把「資料」跟「介面」分清楚

前幾輪的逐頁掃描都跑在**空的**實例上，所以「有資料才出現的表格」
（TEST_PLAN §0.6 列的第三格）從來沒被看過。這次先種示範資料再掃：
英文 71 條、日文 9 條。

**逐條看完之後，一條都不是漏翻** —— 全部是使用者自己的資料：公司資料卡的
欄位名稱與值、群組名稱與說明、印章 / 簽名 / 浮水印的名稱、公司設定頁的每一格。

這幾處標上 `data-i18n="skip"`。**表單自動填寫那張公司資料卡特別重要**：
那些欄位名稱是管理員可以改的，而且同時是拿去比對台灣廠商表單上中文欄位標籤
的依據 —— **翻掉會讓比對安靜失效**。

> **「掃出很多條」不等於「有很多漏翻」**。分不清資料與介面的話，這份報告
> 下次就沒有人看了 —— 而真正的漏翻會跟著一起被忽略。

## [1.15.52] - 2026-09-16

### JS 設定的顯示屬性也要走 `tr()`

`title` / `placeholder` / `aria-label` / `alt` 有兩種來源：樣板裡的
`title="…"`（伺服器渲染時就翻好了，沒問題），以及 **JS 執行期設定的**
`el.title = '…'` —— 後者**樣板掃描看不到**，而且它不是文字節點，
逐頁掃描要「滑鼠移上去」才看得到。

掃下來 4 處，全部是內插的樣板字面值（`` el.alt = `第 ${n} 頁` ``）——
跟上一版對話框那一輪同一個形狀。守門
`tests/test_js_set_attributes_go_through_tr.py`。

> 門檻是**量出來的**（實際 10 處，取一半當下限）—— 寫一個好看的整數會變成
> 「永遠成立」或「動不動就紅」。

## [1.15.51] - 2026-09-16

### ⚠⚠ 送出之後的結果區，英文 / 日文介面上一直是中文

TEST_PLAN §0.6 那張表寫著逐頁掃描**看不到「送出後的結果區」** —— 而那一格
從來沒有人掃過。這次讓**截圖工具**順手掃：它本來就會真的上傳檔案、送出、
等結果出現才拍照，**它眼前那一幕正是缺的那一格**。

第一次跑就抓到 **11 支工具**的殘留（浮水印的預覽狀態列、PDF 編輯器的載入
訊息、OCR 的上傳訊息、逐句翻譯的載入訊息、去識別化的統計、PDF 轉文書檔的
前後對照標題與頁碼、圖片下載鈕的提示…）。

根因全部是同一類：**那幾句話是內插出來的**（`` `第 ${n} 頁` ``），
而內插過的整句永遠查不到字典。有些甚至連 `tr()` 都沒包。

> **`tr()` 加了一層退路**：查不到時把**連續數字換成 `{0}` `{1}`…** 再查一次，
> 查到就把數字填回去。這樣**伺服器送來的作業訊息**（`完成（3 份）` —— 它在
> 背景執行緒裡產生，那時候沒有 request、不知道使用者的語言）也翻得動了。
> 查不到就原樣回傳，**最壞情況跟現在一樣**。
>
> **它的界線也寫成測試**：只處理「變數全部是數字」的句子。
> `已上傳 a.pdf（6379.1 KB）` 的第一個變數是檔名，那種要在產生那句話的地方
> 自己包 —— 不要以為這個退路什麼都接得住。

> **去識別化的樣態名稱是「修過一次、漏了另一半」**：v1.15.47 修的是結果列
> 那個標籤，**統計晶片那一排漏掉**。同一個家族要一次掃完（第 N 次）。

### ⚠ 有些下拉的選項文字**不可以翻**

個資限用章的「用途範本」選到什麼，那串字就**原樣印到章上**
（「僅供○○使用，他用無效」）。翻掉的話畫面上選的是英文、印出來是另一個字，
**而且看起來完全正常**。

那個下拉標成 `data-i18n="skip"`，**掃描器也補上了對 `<option>` 的 skip 檢查**
—— 原本 `<option>` 那一段沒有這道檢查，於是這幾條會變成永遠掛在報告上的
誤報，而誤報一多這份檢查就會被當雜訊忽略。

> **只標在 `<select>` 上還不夠**：全站的下拉是自製元件，**畫面上看到的那份
> 清單是另外建的節點**，不在原本的 `<select>` 裡面 —— `closest()` 找不到那個
> 標記，於是只擋到隱藏的原生 `<option>`，畫面上那一份照樣被判成漏翻。
> 現在元件會把 `data-i18n` 一併傳給它自己的外層。

另外，權限矩陣裡的**帳號名 / 群組名**也標成資料（那是使用者的資料，
中文是正確的），而工具名稱、角色名稱、「群組」這三處**本來就該翻卻沒包**，
一併補上。

### 對話框：靜態掃描一次抓到 15 處沒包 `tr()`

TEST_PLAN §0.6 把對話框列在「只能靠人工逐頁操作」那一格 —— 但**靜態掃描
看得到所有分支**，包括只有出錯時才會走到的。形狀分三種：

* 內插的樣板字面值 —— `` showAlert(`已清除 ${n} 筆鎖定`) ``
* 純字串忘了包 —— `showAlert('兩次輸入不一致')`
* **三元運算只包了一半** —— `` cond ? '已啟用…' : tr('已關閉…') ``
  （最陰險：另一半是對的，看起來像已經處理過了）

守門 `tests/test_dialog_strings_go_through_tr.py` 的判準是
「把第一個參數裡的 `tr('…')` 挖掉之後不可以有中文」——
**不可以只驗「開頭是不是 `tr(`」**，那會把 `cond ? tr(a) : tr(b)`、
`err.message || tr(c)` 這種正確寫法判成違規。

> **我自己的註解又騙過自己的掃描器**（第 N 次）：我在修法旁邊寫的說明裡
> 引用了那個錯誤的寫法當例子，守門當場把它報成違規。掃描前要先去註解，
> 而樣板只能取 `<script>` 裡那幾段（把整份 HTML 丟給去註解的函式，
> 會把內文當程式看）。

## [1.15.50] - 2026-09-16

### 記憶體准入：同一輪派出去的工作要先記帳（稽核 F06）

派送迴圈在同一輪裡可能連派好幾件，而每一件都讀**當下**的可用記憶體 ——
前一件此時**還沒配到記憶體**，所以第二件看到的是過時的數字，兩件都被放行。
預設併行 2 時最多多算一件（800 MB）；管理員調到 4~6 就是 2.4~4 GB，
正好是 OOM 的量。

現在派出去的工作會先在帳上掛著它的估計值，**過了沉澱時間（30 秒）才不算**
—— 那時它實際配到的記憶體已經反映在可用量上，再算進去就是重複計算、
會比需要的更保守。

> **「沒有工作在跑時不做記憶體判斷」維持原樣**：那是刻意的取捨（不然記憶體
> 一直不足時佇列永遠解不開），程式裡本來就有註解。這次只補上它真正缺的
> 那一半。
>
> 驗收的數字是**推出來的**：office 一件估 800、系統保留 768，所以舊做法在
> 可用 ≥ 1568 時放行、新做法要 ≥ 2368。測試取 2000 落在中間。

### 被開成多 worker 時會講出來（稽核 F11）

`OPS.md` 早就寫著「不要開成多 worker」，但**沒有任何東西擋著** ——
而症狀（同一件作業被派好幾次、正在跑的作業莫名其妙變成「已中斷」）
看起來完全不像設定問題。

啟動時偵測到就在記錄裡留一行 ERROR，寫明**是怎麼判斷的**以及**會發生什麼
事**。**只記錄不阻止** —— 在啟動時直接拒絕服務比問題本身更糟。

> 判準是讀 uvicorn 的原始碼確認的（它用 `multiprocessing.get_context("spawn")`
> 起 worker，所以 worker 裡 `parent_process()` 不是 `None`），不是猜的；
> 另外也看 `WEB_CONCURRENCY`。

### 其他

* 原始碼裡不可以有無效的跳脫序列（`\-`、`` \` ``）。Python 3.12 只發警告，
  **3.14 起是 `SyntaxError`** —— 到那時候整個測試套件會在收集階段就掛掉。
  實際踩到的兩處都在**說明文字**裡，改成 raw string 即可。
* 兩支安裝程式守門的「哪些語言的譯文可以有漢字」收成一份共用的判準。
  加日文時只改到其中一支，另一支當場紅 —— 同一個家族要一次掃完。
* i18n 的逐頁掃描加了 `--reveal`：掃之前先把**已經在 DOM 裡但沒顯示**的
  面板與區塊攤開，補上「要點開才看得到」那一類。**執行期才建出來的節點
  （真的按下去才生成的對話框）仍然掃不到** —— 這個方法也有它看不到的東西。
* 「一個標籤對應到多個 canonical key」是**刻意支援的**（台灣表單常有一格
  同時管兩件事），現在有守門釘住 —— 差點就把它當成缺陷「修掉」。

## [1.15.49] - 2026-09-15

### ⚠ 修正後的圖沿著四個邊有一圈紅線（客戶回報）

看起來像「自動抓四個角時把桌面一起框進去了」，**其實不是背景，是我們自己
填上去的顏色**。旋轉補邊用的是

    cv2.warpAffine(..., borderValue=255)

而 OpenCV 的 `borderValue` 收的是四個數：只寫 `255` 會被展開成
`(255, 0, 0, 0)` —— 灰階時那是白色（正確），**彩色時那是純紅**。
v1.15.47 把幾何從灰階改成套在彩色上之後，這一行的意思就悄悄變了。

實測那張名片：純紅畫素 **5,628 個 → 0**，最外兩圈的平均色度
**43.8 → 2.8**（紙面本身是 4.2）。四個角本身抓得很準（墨水涵蓋率 1.000、
純度 1.000），一個畫素都沒抓錯。

順著同一條路還修掉一個更早的順序錯誤：**壓平底色要在旋轉之前做**。
旋轉會在四角補白，那片白不是拍到的內容，卻會被背景估計當成
「這一區本來就很亮」，估出來的增益場跟著被拉低 ——
實測同一張照片：順序錯的時候**暗像素 64.7%**，改成先壓平再旋轉是 **6.5%**。
原本補的是紅色（亮度只有 54，剛好被當成深色區遮掉）才沒發作 —— 也就是這個
順序一直是錯的，只是被上面那個 bug 蓋住；**修掉前者才會看到後者**。

### ⚠ 拉四個角：拉完第一個，圖會往上跳

狀態那一行就在「修正前」那張圖的上面，而它是 flex-wrap 的：文字長的時候
折成兩列、短的時候縮回一列。每次重算都先換成「產生預覽中…」，於是
**拉完一個角圖就往上跳 40~50 px**，剩下三個角全部落在低 19% 的位置。

拉出來的四邊形因此被判成不合理（內角極差 40.4°，上限 40°）而**整組丟掉**，
畫面上卻還寫著「用的是你自己拉的四個角」。

保留那一條的高度就好（**只長不縮**，而且是記住目前為止最高的那一次，
不寫死數字 —— 中文 / 英文 / 日文長度不同，視窗寬度也會變）。

### 工具更名：文件拉正 → **文件擺正**

「拉正」是描述動作的口語說法；這支工具做的是裁邊、拉正、去底色三件事，
輸出的是一份擺正的文件。介面上的動作一律用「修正」。
工具的網址與 API 路徑（`doc-straighten`）**沒有變**。

### 安裝程式：支援日文，而且不再出現亂碼

**亂碼**（客戶回報，Win11 25H2）：`winget` 送出的是 UTF-8，而 NSIS 的
`nsExec::ExecToLog` 用**系統的 ANSI 字碼頁**去解 —— 繁中 CP950、日文
CP932、英文 CP1252，**三種都會亂**。那一整片內容（授權條款、進度動畫）
對使用者本來就沒有意義，一律導進 `installer.log`，畫面上只留我們自己的
一行狀態。

**日文**：原本只宣告繁中與英文兩個語言表，而 NSIS 挑不到對應語系時會用
**第一個宣告的** —— 日文 Windows 拿到的是繁體中文的安裝畫面。
補上日文之後，元件清單、完成頁、失敗訊息與**解除安裝那三句**都有日文
（解除安裝走的是語言對話框之前就返回的那條路，最容易漏）。

### 更新記錄不再引述使用者說的話

原本會寫成「使用者：『…』」這種形式。這次的措辭還算中性，但同一個寫法
下一次可能就把一句抱怨、或一句看得出是誰的話公開出去 ——
**症狀留著**（別人靠那句話搜尋到自己遇到的問題），拿掉的是引號與
「某某人說」那一層框。全份 34 處改寫完畢。

### 介紹站

* 稽核記錄那一條不再把三種格式塞在同一句裡（`syslog / CEF / GELF`
  在下面的功能表與說明段落都還在）。

### 去識別化支援**日文文件**

文件語言多一個「日本語」。日文專屬的樣態六類：**マイナンバー（個人番号）**、
**法人番号**、**電話番号**、**郵便番号**、**住所**、**氏名**。
介面是日文時，文件語言預設就是日文。

* **有檢查碼的一律驗**：マイナンバー 與 法人番号 都有 —— 不驗的話任何
  12 / 13 碼數字都會中，型號與注文番号會被大量誤判，而畫面會顯示「已處理」。
* **誤判語料是必要的驗收**：一份滿是型號 / 注文番号 / ISBN / 版本號的日文文件，
  敏感類別命中必須是 **0**。只驗「抓得到」的話，把式子放寬到抓一切也會過。
* **假值用日文的，而且過不了檢查碼** —— 驗得過的假號碼可能真的屬於某個人
  （同 SSN 用 9xx、IBAN 刻意不通過 mod-97 的原則）。
  電話用 `0120-000-xxx`、郵便番号用未指派的 `〒000-xxxx`。
* **台灣與英文那兩組零退步**（改動前後各掃一次真實語料比對）。

> **⚠ PDF 抽出來的分隔符不是你打的那一個。** 實測 PyMuPDF 對同一份 PDF 給的是
> 不斷行空白 `\xa0` 與**不斷行連字號 `\u2011`**（不是 `-`）—— 用 `[ \-]` 去配，
> 7 類只抓得到 3 類，**而畫面會顯示「已處理」**。本專案在中文地址上記過
> 同一條（「PDF 抽出來的空白不是半形空白」），這是它的連字號版。
>
> 放寬分隔符之後又多出一筆誤判（`社内コード：03-1234-5678-X-99` 被當成電話）
> —— 前後**不可以緊接連字號**那一半不能一起放掉。
> 兩條都釘成驗收，各自做過變異驗證。

### ⚠ 十幾支掃描器的 `</script>` 正規式會漏掉一整片檔案

`</script  >` 是合法的 HTML —— 寫死 `</script>` 的正規式會把它當成「還沒結束」，
**整塊被當成 script 內容吞掉**，那支掃描器從此少檢查一整片。
CodeQL 的 `py/bad-tag-filter` 報的就是這個（11 個 High）。

收成 `tools/source_text` 的一份（`</tag\b[^>]*>`），全站 14 處改走它。
守門只掃**正規式**那幾行，`tests/` 與 `tools/` 一起掃。

> 本專案 issue #15 踩過同一個家族：註解裡的字面 `</script>` 讓瀏覽器提早關閉
> 標籤，整頁 JS 變成純文字。

### PDF 編輯器：斷線 / 誤關分頁之後**救得回來**了

編輯狀態原本只活在那個分頁裡 —— 伺服器上那份是**已經燒進去的 PDF**
（不是編輯狀態），而且 2 小時後被暫存清理掃掉，拿回來也不能再拖動文字框。

現在編輯內容會存進瀏覽器，下次開**同一份檔案**時會問「要不要接續上次的編輯」。

> **換電腦就沒有** —— 這件事寫在提示列上，不要讓人以為它在伺服器上。
>
> **空的編輯狀態不可以蓋掉既有的草稿**：重新開檔時會先對「還沒編輯的空畫布」
> 拍一張快照，那一張在 1.5 秒後就把上一次的草稿洗掉了，
> **使用者根本來不及按「接續」**（實測：存的時候有 1 個物件，重整後讀回來是 0 個）。
> 這一條只有**真的在瀏覽器裡跑一次**才看得到 —— 靜態檢查看得到程式碼在那裡。
>
> **同一份檔案才提議**（比對檔名與頁數）：把 A 檔的編輯套到 B 檔上，
> 版面看起來「好像可以」但位置全錯，而且使用者不會發現。

### PDF 轉文書檔：逐頁進度

`Converter.convert()` 是一個從頭跑到尾的呼叫，中間什麼都看不到 ——
而這支工具動輒數分鐘，使用者只會看到進度條不動。
改成拆開它自己的四步，再從 pdf2docx **自己記的逐頁 log** 取顆粒度
（不碰它的內部結構）。4 頁的文件現在回報 10 次。

> 上游改掉那行 log 的話我們只會**安靜地退回階段進度** ——
> 正因為如此才要有守門盯著那個格式。
>
> **進度壞掉不可以讓轉檔失敗**：它是附屬品，不是產出。

### 介紹站的截圖不再是空狀態

「使用者管理」與「權限矩陣」原本是「還沒有任何使用者或群組」、使用者 0 人。
示範資料多了 6 個帳號、3 個群組、以及**同名帳號在 local 與 ldap 並存**
（那正是那張截圖要展示的東西）—— **全部虛構**。

> 建了使用者就要**明寫「認證關閉」**：`auth_settings.json` 不存在時，
> 產品的 fail-secure 會看到「資料庫裡有使用者」而自動改用本機認證
> （v1.15.34 那條刻意的防護）—— 於是拋棄式實例整站要登入，
> 截圖全部變成登入頁。
...
使用者截圖：`Jason Tools ドキュメントツー / ルボックス` 斷成兩行。

**量出來的原因**：標題不折行時要 中文 **166** / 英文 **225** / 日文 **285** px，
加上導覽列之後英文要 **1186**、日文 **1188** —— 而容器只有 **1180**，
**就是差那幾個 px**。

改成兩行的固定版式：第一行永遠是 `Jason Tools`（每個語言都一樣），
說明放第二行（原本 `jt-doc-tools` 的位置）。
現在三種語言合計只要 919 / 1052 / 1002 px，還有一百多 px 的餘裕。

> **不是把字級調小或把斷點再往上拉** —— 那只是把問題推到下一個語言。
> 把「長度會變的那一段」搬到不影響版面的地方，之後翻成多長都不會再折行。
>
> 守門兩條：①三種語言的第一行都必須是 `Jason Tools`
> ②兩行都要 `white-space: nowrap`。變異驗證各紅一次。
...
介紹站的截圖裡沒有載入範例檔，畫面上是錯誤對話框。根因不是忘了放：

* `/usr/bin/chromium-browser` 是 **snap 版**，它**讀不到 `/opt`** —— 而範例檔
  就放在 `/opt/jt-doc-tools/temp/`。
* `DOM.setFileInputFiles` 照樣「成功」、畫面上的檔名也顯示得出來，
  **只有真的要送出時 XHR 才炸 `network error`，而伺服器端一筆請求都沒有**。

所以這一整批截圖的上傳從上線起就沒有成功過（英文那組停在
「Please upload a PDF first」的對話框也是同一個原因）。CLAUDE.md 慣例第⑲條
記過這件事，我沒有第一時間想到。範例檔改放到瀏覽器讀得到的地方之後，
每一頁都是「工具真的在用」的畫面。

> **同一個家族的教訓**：畫面上「看起來成功」（檔名有顯示）不代表成功。
> 判準改成**看伺服器端有沒有收到那個請求**，以及**產出的截圖裡有沒有蓋著
> 對話框**（現在會印出來）。

### 截圖：用**全部虛構**的示範資料，而且真的跑一次

自動填表的截圖改用示範公司資料，而且是真的跑過一次填寫的結果。

`tools/seed_demo_data.py` 產生一份示範公司（範例科技股份有限公司、統編
12345675、02-1234-5678…**一個字都不是真的**）、一顆示範印章、以及一份合成的
供應商基本資料表。表單自動填寫的截圖是**真的填完 18 個欄位**的畫面 ——
不必再把真實資料塗掉。

每一頁改成逐頁的「配方」（要放哪一份檔、要不要按下去、要捲到哪裡），
並且**等結果真的畫出來**而不是死等固定秒數。

### 台灣專屬的工具不出現在英文 / 日文的介紹站

台灣專屬的那幾支工具在英文 / 日文介面下本來就是反灰的，介紹站還放它們的
截圖只會讓人以為能用。

判準走註冊表的 `ToolMetadata.locales`（**不要自己維護一份名單**），
整個 `<figure>` 拿掉之後編號重排 —— 少一張卻留著 01 / 03 / 04 的跳號更難看。

### 介紹站的語言選單改成下拉

使用者截圖：導覽列上「繁體中文English日本語」黏成一團，而且整條導覽折行。
語言只會越加越多，所以改成下拉（寬度跟語言數無關）。

導覽列同時做了三件事：

* **項目不折行** —— 折行之後會變成「Why self- / host」「なぜ自前で建て / るのか」
  這種斷在詞中間的樣子。
* **日文的項目改短**：インストール → 導入、トラブルシューティング → 困ったときは、
  なぜ自前で建てるのか → 自前で建てる理由。
* **收成漢堡選單的斷點從量測來**（`chromium --headless` 逐語言量
  `scrollWidth`）：導覽列要 中文 805 / 日文 875 / 英文 933 px，
  英文在 1060px 塞不下、1100px 還放得下 → 取 **1080px**。
  原本寫 980px 是照中文長度定的。

### 文件拉正：沒有勾「轉成黑白」，輸出卻是黑白的

使用者拍的是彩色名片。根因是**整條管線都在灰階上跑**
（`crop_page(gray)` / `warp_quad(gray, …)`），彩色從來沒有留下來過 ——
而畫面上那個勾選框寫著「轉成黑白（預設不要用）」，等於**介面承諾了一件
沒有做到的事**。

改成「**量在灰階、做在彩色**」：幾何（透視 / 裁邊 / 旋轉）套在彩色那一張上，
灰階只拿來量角度與找紙邊。順手抓到 **RGB / BGR 沒轉**（PyMuPDF 給的是 RGB、
`imencode` 吃的是 BGR）—— 不修的話紅藍會對調。

> 判準是**飽和度**不是「有幾個通道」：輸出成 3 通道但每個通道都一樣，
> 看起來仍然是黑白的。另外正向釘住「只給灰階時不可以變成彩色」。

### 文件拉正：可以指定輸出尺寸（像素 / mm）

修正後的輸出尺寸可以指定，像素與 mm 兩種單位都收；預設自動帶入偵測到的
大小，使用者可以自己改。

自動（依修正後的大小）／ A4 ／ A4 橫 ／ A3 ／ Letter ／ 自訂（寬 × 高 ＋ 單位）。
預覽回來之後會**先把修正後實際的大小填好**，動過手就不再自動蓋掉。

> **比例對不上時維持比例、四周留白 —— 絕不裁切也不拉伸。**
> 這支工具從第一版就寫著「切到內容不可原諒，多框一條桌面只是難看」。
> 留白補**白色**不是黑色（補黑列印會整片吃墨）。
> 尺寸填壞了一律當成沒填，不可以丟 500。

### 文件拉正：重繪中又有新動作時，舊的那次馬上停

重繪途中又有新動作時，進行中的那次要能馬上停掉。
原本只有一個 `pvToken` 擋「晚到的回應」—— 請求本身照樣跑完，**伺服器白算
一次**。改用 `AbortController` 真的中止。
...
* 「3. 修正前後」的縮圖列原本貼在面板最左邊，跟底下的欄位對不齊 ——
  移進欄位那一欄，頁碼與狀態同一行。
* **動作一律改用「修正」**（「拉正」不是台灣慣用的動作說法）。
  跟這支工具自己的「修正前 / 修正後」一致，也是事務機繁中介面的講法
  「原稿歪斜修正」。不用「校正」—— 這個專案裡「校正」已經是 LLM 逐欄比對的
  意思。工具名稱維持「文件拉正」。

### ⚠ 去識別化：日文介面被當成台灣，信用卡號被認成「市話」

v1.15.46 修的是文件去識別化那一支，**文字去識別化照樣抄了一份舊邏輯**
（「不是 `en` 就當 `zh-Hant`」）—— 日文截圖裡當場看到
`4111 1111 1111 1111` 被認成台灣市話。兩支現在共用
`patterns.default_doc_lang`，守門走 AST 驗「有沒有真的委派過去」。

> 「**同一個家族要一次掃完**」這條教訓，同一天犯了兩次。

同一輪另外兩個：

* **偵測結果的樣態名稱沒走翻譯** —— 英文 / 日文介面下那排標籤全是中文
  （兩支工具都是）。
* **地址遮罩寫死台灣格式** ——
  `1842 Maple Street, Springfield, IL 62704` 被遮成 `OO市OO區OO路OOO號`，
  讀的人會以為那份文件本來就是台灣的地址。遮罩的用途是「保留格式、看不出
  內容」，換成另一個國家的格式就違反了前半段。現在中文地址維持原樣，
  其餘語言逐字遮成 `*`、只留分隔符號與長度。

### 介面

* 「PDF 轉文書檔」拿掉 **Beta** 字樣（使用者 2026-09-14 指示）。
* **升級說明補一句「`-C` 是大寫」**（2026-09-14 客戶回報）：打成小寫的
  `git -c /opt/jt-doc-tools/ …` 會回
  `fatal: not a git repository (or any of the parent directories): .git`
  —— 小寫的 `-c` 是**設定參數**，git 不會切到那個目錄。
  **那個訊息看起來像「這個安裝不是用 git 裝的」，其實只是打錯一個字母**，
  客戶因此往完全錯誤的方向查。README / `OPS.md` / 介紹站三處都補上，
  並加守門擋住「可複製的指令區塊裡出現小寫的 `git -c <路徑>`」。
* 側欄帳號選單的「介面語言」在日文下折行成「日本 / 語」——
  日文改用 UI 的標準說法「**表示言語**」，並加上不折行。

---

## [1.15.46] - 2026-09-14

### 新增日文介面（介紹站 / API 手冊 / 疑難排解 / README 都有日文版）

側欄「介面語言」多一個「日本語」。語系檔 **4,891 條**，公開文件四份
（`index-ja.html` / `api-ja.html` / `troubleshooting-ja.html` / `README_ja.md`）。

**語言切換從「切換」改成「列出其他語言」** —— 兩種語言時一顆按鈕就夠，
三種語言之後，站在日文頁上就沒有去英文頁的出口了。每一頁列出**自己以外的
全部語言**。

> **生成器與守門的語言清單一律讀 `ui_locale.SUPPORTED`**。原本有八個地方
> 各寫死一次 `en` —— 那些守門在加了第三種語言之後會**安靜地只驗英文**
> （「掃 0 個檔」跟「掃過都乾淨」在 pytest 輸出裡長得一模一樣）。

**加日文才現形的三件事**（英文那一輪都不會發生）：

* **用詞守門把日文整份誤判。** 日文的「保存」「字体」是正確的日文，卻正好
  在中文禁用詞清單上（中文要寫「儲存」「字型」）。日文幾乎每一行都有漢字，
  不排除的話整份都是誤報 —— **誤報一多這份檢查就會被當雜訊忽略**。
  判準放在**檔名**上（`README_ja.md` / `index-ja.html`），因為語言版跟中文版
  同一層，用目錄分不出來。
* **語系檔那條「譯文不可以有漢字」對日文不成立。** 日文譯文 100% 有漢字。
  改成抓「現代日文不會用的中文詞」（`這` / `嗎` / `什麼` / `沒有`…）——
  那是「整段忘了翻」的訊號。**`的` 不可以放進來**：「一般的」「自動的」
  是正確的日文（第一版放了，當場三條誤報）。
* **介面有日文不等於去識別化支援日文文件。** 文件語言的預設值原本是
  「不是 `en` 開頭就當台灣」，於是日文介面的使用者套到的是台灣那組規則 ——
  台灣的市話 / 地址 / 統編式子套在別的語言上是**抓錯**不是抓不到，
  而畫面會顯示「已處理」。沒有專屬式子的語言改用與語言無關的那一組。

### 修正：下拉選單的文字沒走翻譯 —— 英文介面從上線起就是中文

逐頁用真的瀏覽器掃日文介面時抓到五處，**每一處在英文介面下也一樣是中文**，
其中翻譯對照字典那兩個下拉從 v1.15.19 上線起就沒對過：

| 位置 | 內容 |
|---|---|
| 翻譯對照字典 | 語言下拉 12 個選項 |
| 系統狀態 | 資料庫清單的名稱（稽核記錄 / 統編資料庫…）|
| 文件去識別化 **與** 文字去識別化 | 文件語言下拉 |
| 文件拉正 | 解析度的說明文字 |
| 登入頁 | 認證來源（本機帳號）|

> **共同點是「文字來自伺服器送來的資料」** —— 樣板掃字面 `tr('…')` 的守門
> 一律看不到。新的守門 `test_option_labels_go_through_tr` 直接驗
> 「`<option>` 裡的運算式有沒有走 `tr()`」，真的是資料的（使用者名稱、
> 工具 id、語言的自稱）列進豁免**並寫下理由**。
>
> **同一個家族要一次掃完**：去識別化那兩支工具各有一份同樣的樣板，
> 我第一次只修了其中一支。

### 修正：介紹站有一句話的語序是錯的（英文版也是）

「需 Office 引擎的工具」那一段被 `<b>` 切成好幾段之後逐段翻譯，兩個動詞
被對調成 *"These tools need Word / Excel / PowerPoint / ODF when handling
OxOffice or LibreOffice"*。**每一段的譯文都要能在它自己的位置上讀得通。**

日文另外補了一條排版規則：行內標記**兩側都是日文**時把中間那個空白收掉
（中文原文在 `<b>` 旁邊常留一個空白，因為標籤裡面是拉丁字），
否則會出現「不要 です」這種東西。

### 修正：語言切換的 cookie 在 https 站台上少了 `Secure`（ZAP 抓到）

`/ui-locale` 用的是 `request.url.scheme == "https"`，而本專案關掉了 uvicorn 的
`proxy_headers` —— **反向代理後面那個值永遠是 http**，所以 https 站台上
`jtdt_locale` 沒有 `Secure` 旗標。改走全站共用的 `is_https_request()`
（順手把 SSO 交易 cookie 也接過去，它原本看的是送給 IdP 的 redirect_uri，
而 `Secure` 該看的是**瀏覽器這一段**）。

> **那支 helper 的說明早就寫著「所有 cookie 的 `secure` 都要走這裡」——
> 但沒有守門。** 這個專案的教訓幾乎都是這樣復發的。
> 新的 `tests/test_cookie_secure_flag.py` 走 AST 驗每一個
> `set_cookie` / `delete_cookie` 的 `secure=`（**刪 cookie 也要帶旗標**，
> `Max-Age=0` 不會沿用建立時的 flags），另有一條驗「豁免的區域變數名
> 真的是那支 helper 算出來的」—— 只驗前半段的話，有人寫死 `is_https = True`
> 也會全綠。
>
> **這條路徑在那之前從來沒被掃到過**：加了日文之後語言切換變成一組連結，
> ZAP 的爬蟲才第一次 POST 到 `/ui-locale`。**「掃出 0 條」跟「沒問題」
> 是兩件事** —— 掃描器只證明了「它走到的那些是乾淨的」。

### 其他

* 截圖工具與逐頁掃描器都改成吃 `--locale`，日文版介紹站用的是**日文介面**
  的截圖（`screenshots/ja/`）。
* `tests/test_i18n_catalog.py` 收 JS 的 `tr()` 鍵時**先去掉註解** ——
  解釋規則的註解裡寫了一個 `tr()` 的例子，就被當成一條真的鍵。
  這是本專案反覆記過的「掃描器被它要檢查的那個名字騙到」，這次是它抓到我。

---

## [1.15.45] - 2026-09-14

### 文件拉正：多頁 / 多檔的**每一頁都看得到**（使用者要求）

原本只有一個**數字輸入框**：要用打字的，看不到有哪些頁、看不出哪幾頁調過，
多檔併成一份之後更分不出哪一頁是哪個檔來的。

改成逐頁縮圖列：點一下切過去並重算那一頁、標出目前在第幾頁、標出
「轉了幾度 / 自拉四角」，多檔時每個來源檔的第一頁有分隔線與檔名。

> **哪一頁是哪個檔來的，從併好的 PDF 本身看不出來** —— 只有上傳當下知道，
> 所以 `/load` 的回應要帶 `sources`（每個來源檔各佔幾頁）。
>
> 縮圖走既有的 `/thumb`（70 dpi、存磁碟快取）＋ `loading="lazy"`，
> 50 頁的文件不會一口氣送 50 個請求。

### 文件拉正：轉向過之後自己拉四個角，產出整個跑掉（使用者回報）

狀態列同時出現「另外轉了 90°」與「用的是你自己拉的四個角」時，修正後的
結果是錯的。**兩個錯疊在一起**：

* 使用者拉的點是在**轉向後**那張圖上（v1.15.44 起「修正前」會跟著轉），
  但呼叫端用**未轉**的長寬把 0~1 換算成像素 —— 轉 90° 時長寬剛好對調。
* 換算完之後，核心又把那組座標**再轉一次**。

轉 90° 的實測：產出 **834×358**（應為 471×629）、墨點比例 **41.2%**
（框到的大半是深色桌面，正確值 1.9%）。

> **修法不是修那兩個換算，是把座標系減成一個。** 現在 `quad` 一律是
> 「**轉向後的正規化 0~1**」，自動偵測也移進 `straighten_page`（轉完才抓），
> 中間沒有任何換算，`_rotate_quad` 整支移除。
> **只要座標系有兩個，遲早會有人在錯的那一邊換算。**
>
> 同一個根因還有一個沒人發現的分支：**回給前端的自動四角也是用未轉的長寬
> 正規化的** —— 轉向後切到手動時，手柄會落在完全不相干的位置，
> 而畫面上「有四個點」，看起來完全正常。現在由核心一併回報。

> **守門兩層**：核心層比對「用轉向後的座標裁出來」與「直接在轉向後的圖上
> 自動抓」的產出（尺寸 ＋ **墨點比例**）；端到端在真瀏覽器裡轉 90°、拉四個角，
> 然後**把修正後那張圖畫進 canvas 量暗像素比例**。
> 只驗四邊形的位置是不夠的 —— 變異驗證確認過，那樣這個 bug 照樣全綠。

> `detect_quad` 在核心刻意**預設 False**：這支原本的語意就是
> 「`quad=None` ＝ 不做透視校正」，改成預設 True 會把每一個既有呼叫的行為
> 無聲換掉。

### 文件拉正：切回「自動抓」沒有真的切回去（使用者回報）

切到「自己拉四個角」、拉完再按回「自動抓」，右邊的修正結果與四邊形範圍
**都還停在手動那一版**。兩個原因疊在一起：

* 切模式只呼叫 `renderQuad()`（那支只管疊圖的顯示），**沒有重跑預覽**。
* 就算重跑，`overrides[頁].quad` 還留著，送出去的仍然是使用者拉的座標。

> **模式是開關不是刪除鍵。** 修法是自動模式下不把那組座標送出去
>（預覽與送出**兩邊都要濾**，只濾一邊的話畫面寫著「自動抓」、
> 交出去的檔案卻是手動的，而且完全看不出來），
> 但座標**留著** —— 切回手動時使用者拉好的點還在，不會白拉一次。
>
> 判準是「切回自動之後的狀態列跟第一次自動算出來的**一模一樣**」——
...
### 文件拉正：四個角的連線**從來沒有畫出來過**（使用者回報第二次）

v1.15.43 把線加粗了，回報仍然是四個角沒有連線。根因跟粗細無關：

* **`SVGElement` 沒有 `hidden` 這個 IDL 屬性**（規格把它定義在 `HTMLElement`
  上）。`svg.hidden = false` 只是在物件上掛一個沒人看的屬性，
  **`hidden` 那個標記一個字都沒動**。
* 而 `platform.css` 的 `[hidden] { display: none !important; }` 是**作者樣式**、
  沒有命名空間限定 —— 瀏覽器內建的 `html.css` 有 `@namespace`（只管 HTML
  元素），我們這條沒有，**SVG 照樣被蓋掉**。

所以那個四邊形從第一版起就永遠 `display:none`，而且**沒有任何 JS 例外**：
旁邊四個手柄是 `<div>`，`.hidden` 正常，畫面上就是「點在、線不在」，
看起來像還沒做而不是壞了。改用 `toggleAttribute`（兩種元素都成立）。

> **兩支守門**：`tests/test_no_svg_dot_hidden.py`（靜態，全站掃「用 `.hidden`
> 開關 `<svg>`」）＋ `tests/test_doc_straighten_quad_overlay_e2e.py`（在真的
> 瀏覽器裡上傳一張合成照片、切到手動、**量那條線在畫面上有沒有寬度**）。
> 只驗 `points` 有值的話，這個 bug 照樣全綠。

> 靜態那支第一版**被我自己寫的註解騙了** —— 說明裡引用了那個不可以用的寫法。
> 去註解的邏輯全站原本有三份，已收成共用的 `tools/source_text.py`
>（會認行尾註解，而且跳過字串與樣板字面裡的 `//`）。

### 文件拉正：新增「清晰化」（預設開啟）

手機翻拍最常見的問題是半邊有陰影。**判準是文字辨識率，不是看起來乾不乾淨**：
...
> **二值化不是清晰化**：同一組素材上局部二值化把辨識率打到 **0.108**，
> 而且**看起來最乾淨的那張正是最糟的那張**。它維持獨立選項、預設關閉。
> CLAHE 提對比也一律更差，不用。

> **不可以用 `divide(圖, 局部最大值)`** —— 辨識率一樣好，但會把整片深色內容
> 洗成純白：一塊深灰照片區平均亮度 **66.3 → 254.9**，整塊不見了。
> 改成估一個**有上下限的增益場**：大片深色區域的背景估計值不可信，
> 標成遮罩用 `inpaint` 從**邊界**補（用大範圍模糊補會把陰影的階梯一起抹掉，
> 辨識率從 0.982 掉回 0.472）。實測深灰區 66.3 → 67.2，幾乎不動。

> **對已經很平的掃描件是 0 變動**（最大變動 0 個灰階）——
> 白點那一步一定要夾「只提亮不壓暗」，不夾的話純白會被壓成 245、
> 97% 的像素被改。這條是「預設開著」的前提。

### 文件拉正：一次拉多個檔案、轉向時「修正前」也跟著轉

* 多檔上傳會**依上傳順序併成一份 PDF**，每一頁都各自處理。
* 按了轉 90° / 180° 之後，左邊的「修正前」縮圖原本不動，跟右邊對不起來。

### PDF 編輯器：開啟有數位簽章的檔案會先講清楚

編輯並存成新檔之後**簽章一定失效** —— 簽章涵蓋整份檔案，
任何改動（即使只是重新存檔）都會讓閱讀器顯示「簽章無效／文件已被變更」。
偵測到簽章時在編輯區上方顯示提示，並指出要保留簽章就得用原檔或重新請人簽章。

### 騎縫章：章外圍的透明留白會讓章變小（外部建議指出，實測重現）

設「章寬 40 mm」時，我們是把**整張圖**縮成 40 mm —— 而拍照去背的章外圍常
留著一圈透明邊，於是紅墨實際只有 **26.8 mm**（留白 25% 時），
**同一個設定在不同來源的章上會得到不同大小**。使用者只看得到「章怎麼變小了」。

合成與拼章預覽現在走同一支 `load_stamp()` 先裁掉透明邊。
**雜點要擋**：拍照去背後角落常留幾個半透明的點，只取 alpha 的 bounding box
會被那幾點綁住、等於沒裁。

> 同一份建議的其他幾點對照程式碼**本來就成立**：切片前先旋轉完整的章、
> 切片寬度用累進取整（不會缺縫或重疊）、有「拼回完整印章」的預覽、
> 沒有任何人工鋸齒／撕紙效果、貼齊的是 `CropBox` 不是 `MediaBox`
>（已補一條正向守門釘住）。

### PDF 編輯器：那個簽章提示原本是**沒有樣式的純文字**

我用了 `class="warn-box"`，而且在註解裡寫「（全站既有樣式）」—— **全站沒有
這個類別**，我發明了它還替它背書。`tests/test_template_css_is_effective`
在完整測試時抓到。已收成共用的 `.warn-box`（`.info-box` 的琥珀色警告版），
兩個都放 `platform.css`，不要在工具自己的 `<style>` 裡再發明第三個名字。

### 騎縫章：預覽可以點下去看大圖（使用者要求）

逐頁預覽與「拼回去長這樣」都可以點開放大，支援左右鍵翻頁與 Esc 關閉。

> **「開得起來」不等於「看得到大圖」**：第一版接上去之後實測開出來是
> **300×424**，跟縮圖一樣大 —— 逐頁預覽本來就是 78 dpi 的縮圖，
> 原尺寸顯示等於沒放大。端點本來就有 `large=1`（150 dpi），改成
> **點開時才去要**（事先幫每一頁都算一份的話，52 頁的文件會回到
> 「每個預覽請求 90 秒」那個老問題）。現在實測 645px → **1240px**。

> **沒有再寫第 14 份**：全站盤點發現已經有 **13 份各自實作**的放大檢視，
> 每一份的鍵盤操作與關閉方式都不太一樣。收成共用的
> `static/js/lightbox.js`（宣告式 `data-lightbox`，用事件委派所以
> **之後才動態塞進來的縮圖也吃得到**），守門
> `tests/test_one_shared_lightbox.py` 擋住再長出新的一份；
> 既有 13 份列成有理由的例外，清單本身也有守門防止過期。

---

## [1.15.43] - 2026-09-14

### 新增：安裝與升級疑難排解頁，失敗時會給網址（使用者要求）

**只印一行錯誤等於把人丟在原地。** 這個專案的失敗多半有明確解法（缺 git、
企業 TLS、磁碟不足、標籤衝突、服務佔住檔案、反向代理寫死協定），但使用者看不到。

* 新頁 `docs/troubleshooting.html`（中英各一份，**每一則都是真實發生過的**：
  症狀 / 原因 / 怎麼辦），最上面有可點選的目錄。
* `install.sh`、`install.ps1`、Windows 安裝程式的失敗訊息、以及 `jtdt update`
  的每一條失敗路徑（fetch / reset / uv sync / 健康檢查）都會印出那一頁的網址。
* **網址跟著作業系統語言走** —— 中文系統給中文頁，其餘一律英文頁。
  CLI 的訊息本身維持英文（純文字 TTY / Server Core 渲染不出中文）。

> **英文頁原本連的是中文頁**（導覽上的 API 那顆）—— 讀者一點就掉回中文。
> 生成器現在會把站內連結一併改寫成英文版，**語言切換那一條除外**
> （它的用途正是連回中文版）。守門 `tests/test_troubleshooting_page_is_reachable.py`
> 六條：頁面存在且寫了那幾個已知狀況、站台連得到、英文頁只連英文頁、
> 兩份都有語言切換、安裝腳本與更新流程的每條失敗路徑都印得出網址。

### 文件拉正：四個角要連成看得見的線

原本 `stroke-width: .6` ＋ `vector-effect: non-scaling-stroke` 的意思是
**0.6 個畫素** —— 疊在照片上幾乎看不見，畫面上只剩四個圓點（使用者回報）。
改成畫兩層（底下白、上面藍），深色桌面與白牆都看得見。

---

## [1.15.42] - 2026-09-14

### 文件拉正：按鈕位置與重算時的狀態（使用者回報）

* **「開始拉正」移到「修正前後」下面** —— 它要在使用者**看過**修正結果之後才按，
  位置就是它在流程裡的位置。選項區只留「預覽這一頁的修正結果」（它產生的正是
  下面那一塊）。
* **拖曳四個角之後，右邊立刻進「重新計算中…」＋ spinner。** 原本要等伺服器算完
  才換圖，那幾秒畫面上掛著的是**上一次**的結果 —— 看起來像「拉了沒反應」，
  使用者會再拖一次。
...
回報的症狀是「我拉的預覽點十字準星位置跟實際落點位置不一樣」。量出來的根因：

「修正前 / 修正後」兩張圖為了**左右等高**用了 `height: 46vh` ＋
`object-fit: contain`（v1.15.39 為了版面加的），比例不同的照片就會在上下留白
—— 實測 **541×972 的框裡，圖只畫了 541×766，上下各空 103px**。而手柄、四邊形
與放大鏡**全部用框的比例**換算：

| | 修正前 | 修正後 |
|---|---|---|
| 手柄放在 40% 高度，實際落在原圖的 | 37.3%（**差 23.65 px**）| 40.0%（差 0.0 px）|
| 放大鏡準星底下的原圖畫素 | 跟手柄不同一點 | **與期望完全相同** |

**這不只是放大鏡不準** —— 送去伺服器的四個角座標也是同一組數字，所以裁出來的
紙緣會整片偏掉。

修法：疊圖層搬進一個 `#dsArt`，由 `layoutArt()` 依 `contain` 的算法擺到
**圖真正畫出來的那一塊**上面（圖載好、換頁、切模式、視窗縮放都重算一次），
裡面的一切維持 0~1 正規化座標。放大鏡的背景尺寸改用那一塊的尺寸（比例才對），
取樣點的偏移也從 64 改成 **62** —— 放大鏡是 128px 的 border-box、邊框 2px，
而背景原點是 padding box，準星在它的正中央。

> **既有守門一條都抓不到**：元素都在、沒有 JS 例外、沒有殘留中文，截圖也看不
> 出來（手柄看起來就在紙角上）。新增
> `tests/test_doc_straighten_overlay_geometry.py` —— 在真的瀏覽器裡把手柄拖到
> (0.30, 0.40)，**量手柄落點與放大鏡準星底下的原圖畫素**，判準是畫素級吻合。
> 變異驗證：把座標系改回圖框 → 立刻報「手柄位置偏了 23.65 px」。
>
> 素材要**刻意用跟圖框比例不同的圖** —— 比例一樣就不會留白，這條測試會變成
> 「掃 0 個檔也全綠」那種假通過，所以測試裡另外釘了一條「這張素材真的有留白」。

### 乘車證明整理：支援 Uber（使用者提供兩份真實檔案）

**只收「行程收據」** —— Uber 一趟行程會有兩種 PDF，而「電子發票證明聯」開的
只有處理費那幾元（計程車行程本身不開電子發票，收據上就這樣寫著）。拉錯的話
表格上的金額會是 10 元而不是車資，所以**畫面上直接寫明要拉哪一份**。

抽得到：日期、上下車時間、起訖地址、總計、**車牌**、**里程**（後兩個是新欄位，
預設不顯示；里程在很多公司是計程車報帳的必填欄位）。

> **時間要取上下車那一組。** 收據上有四個時間 —— 叫車、上車、下車、付款
> （實測 16:58 / 17:01 / 17:27 / 17:28）。直接抓第一個會拿到**叫車時間**，
> 只差三分鐘、看起來完全合理，不會有人發現。判準是「時間的下一行是地址」，
> 只有上下車是這個形狀。
>
> **地址只清掉確定是雜訊的部分**（國碼、郵遞區號、重複一次的城市名）——
> 里名與樓層對報帳的人可能有用，不要自作主張刪。

### ⚠ Uber 的收據**沒有票號** —— 同一天的第二趟會被安靜丟掉

清單去重一向走票號（鐵路票都有）。Uber 沒有，於是退到
「交通工具＋日期＋起訖＋車資」—— **同一天同路線同車資的第二趟就被判成重複**。
通勤來回、車資一樣，這在計程車上很常見，而且丟掉是無聲的。

收據裡其實有唯一識別：第一頁那個 `riders.uber.com/trips/<行程編號>` 連結。
**它在文字層看不到，只在連結註解裡** —— 抽文字時一併抽連結就拿得到。
連結掉了（收據被重新列印）時退路是把出發時間也算進去。

> 順手處理了「萬一真的拉進電子發票」：**併進同一天那趟行程**，只補發票號碼與
> 買方統編，**不另外產生一列** —— 行程收據的「總計」已經含了處理費
> （413 ＋ 10 − 13 ＝ 410），多一列就是報帳金額重複計算，而且畫面上兩份檔案
> 兩列看起來完全合理。對不到行程時才留成獨立一列（不可以安靜吞掉）。
> 分兩次上傳也會併。

### ⚠⚠ 文件拉正：掃描件不該被透視「校正」（下載 82 張公開語料實測）

先前只用使用者給的兩張手機照片與合成樣本測過。抓了 82 張真實的掃描件與翻拍
照片跑全套量測，**才看到四邊形把掃描件最上面的抬頭文字切掉**。

根因又是遮罩（頁面最上緣色調不同被判成背景）。與其再調遮罩，判準改成認清楚
**這種圖本來就沒有透視可以校正**：挑出來的候選是完美矩形（內角極差 0°、
對邊等長）而且佔滿大半畫面時，透視變換唯一會做的事就是裁掉邊緣。

| | 佔畫面 | 內角極差 | 對邊差 |
|---|---|---|---|
| 掃描件 / 正面照（16 張）| 0.42~1.00 | **0.0°** | **0.000** |
| 真的有透視的翻拍（4 張）| 0.20~0.85 | 8.4~32.8° | 0.12~0.56 |

兩張真實翻拍照片完全不受影響（墨水 0.998 / 1.000、純度都 1.000）。

> **量測本身也要有對照組。** 我先做了一版「整張圖最暗的 N% 當墨水」的指標，
> 兩張**裁切完全正確**的照片只拿到 0.015 / 0.036 —— 因為那些照片的桌面比紙暗，
> 抓到的是桌子不是字。換成「比周圍明顯暗」仍然被紙只佔畫面兩成的情形騙過。
> **換分母是在猜**；最後是把圖畫出來看才定位到真正的問題。

---

## [1.15.39] - 2026-09-13

### ⚠⚠ 拉正之後上下還留著一圈桌面 —— 根因不是四邊形，是**遮罩漏掉陰影裡的紙**

回報指出紙張邊界沒有完全抓到。把遮罩畫出來看才確定：
**落在陰影裡的那半張紙被判成桌面**（Otsu 只蓋到畫面的 32%，紙實際佔 40%），
於是四邊形只框到「被照到的那一半」，而最小外接矩形為了把它包回來就多框了
一條桌面 —— 那就是畫面上看到的黑邊。

| | 舊（Otsu ＋ 最小外接矩形） | 新（Otsu∪色度 ＋ 候選評分） |
|---|---|---|
| IMG_2904 | 墨水 0.738 / 純度 0.909 | 墨水 0.737 / **純度 1.000** |
| IMG_2905 | 墨水 0.747 / 純度 0.925 | **墨水 1.000 / 純度 1.000** |

三件事：

* **遮罩加一層色度**：紙是中性色（Lab 的 a/b 接近 128），木頭 / 橘色桌墊
  偏離很多，**而影子不改變色相、只改亮度**。兩張遮罩各自做完形態學再取聯集。
* **候選要有好幾個再挑**（借 OSS 文件掃描 app 的路數）：`approxPolyDP` 掃
  epsilon 找四點 ＋ 最小外接矩形。
* **判準寫成兩個可以量的數**：**墨水涵蓋率**（紙上的字有多少被框進去）與
  **純度**（框裡面有多少真的是紙）。先要求墨水不可以比最好的差 1% 以上
  （切到字是不可原諒的），在這個前提下挑純度最高的。

> **亮度門檻要相對於「確定是紙」那一塊，不可以用整張圖的百分位。**
> 合成一張重陰影的樣本才抓到：陰影裡的紙 L=118，而整張圖的第 25 百分位是
> 122 —— **陰影面積一大，百分位自己就把判準廢掉了**。改成「紙的中位亮度
> × 0.45」，係數掃過 0.30/0.40/0.45/0.55/0.65，**0.45 以下三個樣本全部滿分，
> 0.55 起崩**。
>
> **殘留角仍然不能當判準**（同 v1.15.37）：把四邊形縮進紙內會切掉內容，
> 而殘留角看起來完美。所以這一輪的判準是墨水涵蓋率與純度，而且**最後要看圖**
> —— 指標對了還是要確認畫面上沒有桌面。
>
> 參考過使用者提供的三個專案：**OSS-DocumentScanner 是 OpenCV 路線**
> （偵測不靠模型），可以借鏡；**UVDoc / DocRes 是深度學習模型**（要下載權重、
> 實務上要 GPU），跟這支工具「不用 AI、不用 GPU、0.8 秒／頁」的定位衝突，不採用。

### ⚠ 兩支作業測試等錯東西（**只動測試，產品沒改**）

完整套件連兩輪各紅一支、**單獨跑都是綠的** —— 同一個形狀：

```python
assert _wait(lambda: j.status == "done")   # 等記憶體狀態
assert j.meta["workspace"]...              # 讀「之後」才會發生的東西
```

`_run()` 的收尾順序是刻意的：`status = "done"` → **先把最終狀態寫進 DB** →
之後才自動存入工作區並補寫 `meta`。等到 `status` 就往下讀，正好撞上中間那段
窗口；機器一忙（完整套件、瀏覽器測試同時開著 chromium）複製檔案要花上幾百
毫秒，窗口就寬到必中。

兩支都改成**等自己真正要驗的那個東西落地**。全站掃過同一個形狀，第三處
（`test_job_queue.py` 讀資料庫那段）本來就寫對了。

> 這是 v1.15.18 記過的那條的第二、三次：**等待要等到你真正要驗的那個東西。**
> 判斷指紋也一樣 —— **單跑全綠、合跑失敗**。

### ⚠ 改公開樹的 `.gitignore` 沒有用 —— 它每次同步都被重寫

調整同步設定時踩到：`github/.gitignore` 是 `sync-to-github.sh` 每次用 heredoc
**重新產生**的，直接改那個檔，下一次同步就蓋回去，**而且完全無聲**
（同步之前測試是綠的，之後才紅）。

判準要落在**唯一來源**上：規則寫進同步腳本，守門驗的是腳本而不是產出
—— 只驗產出的話，下一個人改錯地方一樣不會有人發現。

> 另一條相關的：`git ls-files` **只看得到已經被追蹤的檔案**，剛放進去還沒
> commit 的東西在它眼裡是乾淨的，而 `rsync -a --delete` 會把它帶進 clone。
> 要確認「某類檔案沒有進到公開樹」，判準得直接掃檔案系統。

### 文件拉正的介面（使用者截圖回報）

* **「自己拉四個角」一眼看不出來可以按** —— 那是一個小勾選框。改成兩張
  `option-card`（「自動抓」／「自己拉四個角」）的二選一，跟站上其他工具
  （例如 PDF 轉圖片的解析度）同一個視覺語彙。
* **處理解析度**也從下拉選單改成卡片，把「快 / 平衡 / 細節」直接寫在上面。
* **說明文字不要不必要地換行**：`max-width` 從 62ch 放寬到 96ch ——
  右邊明明還有空間卻硬折成兩行。

> **改成 radio group 之後取值要跟著改**：`$('dsDpi').value` 那個 id 已經不存在，
> 拿到的是 `undefined` **而且完全不會報錯**。另外選「自動抓」時不會觸發
> 「自己拉」那顆的 `change`，事件要掛在整組上。

---

## [1.15.38] - 2026-09-13

### ⚠⚠ 同一個根因，連續四次：用了在那個情境下**不會生效**的類別

連續收到兩則回報：版面不好看、下拉清單沒套用本專案樣式。逐一查下來全部是
同一件事，而且**畫面上不會有任何錯誤訊息**：

| 用到的 | 實際情況 |
|---|---|
| `class="notice"` | 全站**沒有任何規則**定義它（我自己發明的名字）|
| `af-field` / `af-note` | `platform.css` 裡只有 `.auth-form .af-field` —— 工具頁不在 `.auth-form` 底下，**等於完全沒有樣式**（標籤、控制項、說明擠成一行）|
| `jt-select` | 那是 `custom_select.js` 的**選擇器掛勾**，而去識別化那兩頁**沒有載那支 js** → 瀏覽器原生下拉 |
| `btn-secondary` | **全站沒有任何規則** —— 共用的作業進度元件（「處理新檔案」）與工具按鈕都掛著它，跟一般按鈕長得一模一樣 |

修法：文件去識別化 / 文字去識別化 / 文件拉正三支改用全站的
`.form-row` ＋ `.field` ＋ `.muted`；`.btn-secondary` 在 `platform.css` 補上定義。

### 這一類以後測得出來：`tests/test_template_css_is_effective.py`

逐頁檢查每個類別在**那一頁的情境下**是不是真的有樣式：

* 全站任何一份 `.css` 裡有規則嗎？
* 有的話，規則要求的**祖先類別**這一頁拿得到嗎？（`.auth-form .af-field` 要求祖先有 `.auth-form`）
* 或者它其實是 **JS 掛勾**（`querySelector('.x')`）、或這一頁自己 `<style>` 定義的？

三者皆非 → 這個類別在這一頁不會生效。既有欠帳列成例外清單，**新工具不准再增加**
（使用者要求：「這種情況必須列入測試計劃，而且以後新工具也要生效」）。

> **守門第一版被自己的解析騙了**：`class="tile-color-{{ t.color }}"` 這種
> **渲染時才組出來**的名字，去掉 `{{ }}` 之後只剩半截，一次誤報 16 支。
> 判準補上「它是某個真類別的前綴就放行」。

### ⚠ 作業自動存入工作區的測試等錯東西（**只動測試，產品沒改**）

完整套件跑到最後一支紅：`test_end_to_end_through_job_manager`，**單獨跑卻是綠的**。

重現方式是把它跟瀏覽器測試放在一起跑（那支會開 chromium、把機器壓住）——
必紅。加診斷之後看清楚：**產出檔在、`result_path` 指得到、工作區是開的，
`meta` 卻是空的** —— 不是逾時，是**讀得太早**。

`_run()` 的收尾順序是刻意的：`job.status = "done"` → 先把最終狀態寫進 DB
→ **之後**才自動存入工作區並補寫 `meta`（那一段有註解說明為什麼要先寫 DB）。
測試等到 `status` 就立刻讀 `meta`，正好撞上中間那段窗口；機器一忙，複製檔案
要花上幾百毫秒，窗口就寬到必中。

改成**等 `meta["workspace"]` 出現**。同 v1.15.18 那條：
**等待要等到你真正要驗的那個東西落地。** 全站只有這一支是這個形狀（掃過）。

### Windows 安裝程式：解除安裝成功卻回傳非零離開碼

在 `.154` 實測「解除安裝 → 全新安裝」時抓到：**無介面解除安裝**（`/S`）完全成功
（服務、登錄檔、安裝目錄全清，防火牆規則移除，**使用者資料與四個 sqlite
完整保留**），離開碼卻是 **2** —— 交棒給 `%TEMP%` 那一份之後 NSIS 的 `Quit`
預設回報「被腳本中止」。腳本化的解除安裝（MDM、`Start-Process -Wait`）會
判定失敗。顯式 `SetErrorLevel 0` 修正，守門變異驗過。

> 這一版**沒有打 tag**，所以這個修正會隨下一支安裝程式出去。

### 安裝程式的兩條路都實測過（使用者要求）

先前只驗過「升級既有安裝」。這次補完另一半：解除安裝（驗資料保留）→
**全新安裝**（131 秒、離開碼 0、簽章 `Valid` ＋ `CN=SignPath Foundation`、
服務 Running／自動啟動、healthz `{"ok":true}`、首頁版本正確、**舊的使用者
資料被接手**）。結果記進 `TEST_PLAN.md` §0.3。

### ⚠ 同一排按鈕，有的有圖示有的沒有（使用者截圖回報）

文件拉正的四顆旋轉鈕，前兩顆有圖示、後兩顆沒有。**元素都在、沒有例外、
也沒有殘留中文**，既有守門一條都抓不到。全站實算後同樣的情形還有 15 排。

順手抓到同一家族但更糟的一種：**按鈕有圖示，JS 又用 `textContent` 覆寫整顆
按鈕** —— `<svg>` 會一起被洗掉。文件翻譯的「停止翻譯」是在顯示的那一刻被洗的，
所以那顆按鈕的圖示**從來沒有出現過**；文字去識別化的「複製到剪貼簿」按一次
之後圖示就永久消失。文字現在放自己的 `<span>`，JS 只改那個 span。

守門 `tests/test_button_icons_are_consistent.py` 兩條。

> **「同一排」一定要用位置判斷**：我第一版用 `<button.*?</button>` 連續比對，
> 非貪婪會跨過中間的 `</div>`，把根本不相鄰的按鈕算成一組（誤報 39 組）。
>
> **第一次變異沒紅要去追為什麼**：我原本規定「兩個中文字以上才算動作鈕」，
> 而「轉 180°」只有一個字 —— **這條守門原本抓不到使用者回報的那一顆**。

### ⚠ 英文文件停在 34 個版本以前，而三條守門全是綠的（使用者回報）

`CHANGELOG_en.md` 最新條目停在 1.15.4（2026-09-05）。既有守門只驗
「檔案在不在、有沒有殘留中文、語言連結指不指得回去」—— 一份過期 34 版的文件
**那三條全部都通過**。又是同一句：記了規則但沒有守門，等於沒記。

* 補上 1.15.7 ~ 1.15.38 共 32 條英文摘要。
* 三條新守門（判準都能自己算，不寫死期望值）：
  **①** `CHANGELOG_en.md` 最新條目的版本號必須等於中文版；
  **②** `README_en.md` 標題的版本號必須等於 `README.md`；
  **③** **把產生器重跑一次、逐位元組比對** —— 英文版介紹站與 API 手冊
  只要落後中文版一個位元組就紅（判準不可以是「有沒有記得跑產生器」）。

### 文件拉正：排版與載入的細修

* **預覽圖不再露出瀏覽器的破圖 icon** —— 圖沒好之前用骨架 ＋ spinner 佔位，
  `onload` 才換上，失敗給人話（「這一頁的原稿載不出來」）。
* **說明文字限寬 62ch** —— 1000px 寬的整行灰字讀起來像一面牆。
* **修正前後兩張等高**（固定高度 ＋ `object-fit: contain`），左右高度不一樣時
  整組看起來是歪的。
* 「重新自動抓」從說明文字裡搬出來自成一列；頁碼欄縮成 90px。
* 轉向時**不再清掉使用者拉好的四個角** —— 使用者是在未旋轉的原稿上拉的，
  後端會把四個角跟著頁面一起轉，丟掉等於把他拉了半天的東西無聲清空。

---

## [1.15.37] - 2026-09-13

### ⚠⚠ 安裝程式：升級既有安裝必定失敗，而且失敗之後掛住不退出

**第一次真的照「打完 tag 就測 Release 上那支安裝程式」做，就抓到兩個。**
簽章是好的（`Valid` ＋ `CN=SignPath Foundation` ＋ DigiCert 時戳），但：

**① 升級既有安裝必定失敗。** `Fetch-Code` 只在「不是 git repo」那條分支停服務，
而**升級走的是 git 那條**（提早 return）。服務還跑著 → `uv venv --clear` 刪不掉
被 `python.exe` 佔住的 `.venv` → 停在 `[X] uv venv failed`。
**這是每一個「裝到既有安裝上」的客戶都會踩的路徑。**

**而且失敗不只是「裝不起來」，是「把既有的環境弄壞」** ——
`setup-python.cmd` 跑的是 `uv venv --clear`：它**先把 `.venv` 清掉**，
然後才在檔案被佔用時失敗。實測之後 `.154` 的 `site-packages` 是 **0 個套件**，
服務一重啟就 `ModuleNotFoundError: No module named 'jinja2'` 起不來
（舊行程還活著時看不出來，所以當下以為只是「安裝失敗」）。

**② 失敗之後掛住不退出。** 失敗路徑的 `MessageBox MB_ICONSTOP "$(ERR_INSTALL)"`
**沒有 `/SD`**。NSIS 的對話框在無介面模式下照樣會顯示，只有 `/SD` 指定的預設答案
能讓它不等人 —— 無介面環境沒有桌面可以按，於是**永遠等下去**：setup.exe 掛著、
CPU 0、使用者只看到「什麼都沒發生」（實測卡了 30 分鐘）。

> **診斷順序**：行程還在 → 看**安裝目錄最後寫入時間**（停著不動）→ 找它自己的
> `Logs\installer.log`，最後一行就是答案。**光看「行程還在」分不出是慢還是卡住。**

修法：停服務移到 `Fetch-Code` 的進入點（兩條分支都停），而且**要等行程真的放掉
檔案握把**；五個 `MessageBox` 全部補上 `/SD`。守門
...
做法（Canny 找邊 → `approxPolyDP` 近似成 4 點），**兩張都回「沒抓到」** ——
而那正是這支工具主打的情境。

原因不是找不到紙：最大的輪廓**就是紙**（佔畫面 29% / 44%），但真實照片的角落
有陰影與圓角，2% 容差近似出來是 **6 個點 / 5 個點**，被寫死的 `len(ap) == 4`
直接丟掉。**合成樣本一張都沒有這種特性**（角落乾淨、近似必中 4 點）。

改法與**踩過的兩個坑**：

* **先去陰影再分割是錯的**：那會把桌面也一起提亮，亮區從 21% 爆到 58~83%，
  四個角直接跑到畫面邊界。
* **用凸包近似成 4 點也不夠**：陰影裡的那一角會被吃掉，四邊形縮進紙內、
  **內容被切掉**（實測筆記左邊一整行字不見）—— 而**殘留角仍然是 -0.05°，
  看起來完美**。

> **殘留角量的是「裁出來那塊正不正」，不是「有沒有抓對紙」。**
> 這一輪有三次是靠**把產出打開來看**才發現抓錯的（§0.5 那條規則）。

現在自動模式走**亮度分割 ＋ 最小外接矩形**：紙本來就是矩形，三個角就定得出
第四個，陰影吃掉一角也不會縮水。代價是斜著拍時會多框一條桌面 ——
**切到內容是不可原諒的，多框一條桌面只是難看**，而要精準貼齊紙緣有
「自己拉四個角」那條路。

另外借了 `andrewdcampbell/OpenCV-Document-Scanner`（640★）的
**四個內角極差** 判準（`quad_angle_range`，上限 40°）：用今天真實失敗的座標
驗過 —— 抓對的 9°、框到桌面的 44°、整個畫面的 44°，擋得住。

兩張照片現在都抓得到紙、殘留 **-0.05°**、內容完整沒被切。
（照片含個資，只放 `temp_pdfs/`，測試在沒有樣本時誠實 skip。）

### 文件拉正：可以自己拉四個角、逐頁手動旋轉

- **自己拉四個角**：勾起來之後在「修正前」的圖上拖四個圓點。**自動抓到的位置會
  先填好**，通常只要修不準的那一兩個。拖曳時有**放大鏡** —— 手指 / 游標一定會蓋住
  要對齊的那個角，沒有它等於在瞎拉。
- **逐頁手動旋轉**：左轉 / 右轉 90° / 180° / 回到原方向。掃反了或掃成橫的用這裡 ——
  **自動拉正只處理 ±6° 的歪斜，方向是另一回事**。轉向之後仍然會自動拉正。
- 座標一律**正規化 0~1**：預覽用 200 dpi、輸出用 300 dpi 是不同的像素尺寸，
  存像素的話換個解析度就整組跑掉。

> **開發時抓到的**：四個角是在「轉之前」的影像上算的，**轉完座標就對不上**。
> 而 `quad_is_sane()` 會把對不上的四邊形擋掉 → **看起來沒事**，其實是使用者拉的
> 四個角被安靜地丟掉，比算錯更難發現。現在四個角跟著整頁一起轉，
> 並用 `cv2.rotate` 的實際結果逐一驗證三種角度。

### 文件拉正的卡片樣式（使用者截圖回報）

選項卡片用的 `af-field` / `af-note` **只在 `.auth-form` 底下才有定義** ——
工具頁用了等於完全沒有樣式，標籤、控制項、說明全部擠在同一行。改用全站的
`.form-row` / `.field` / `.muted`；拿掉重複的「完成」卡片（共用元件本來就有那四顆鈕，
兩份疊在一起才會「幾乎貼在一起」）；按鈕列補間距。

> 這是同一支模板第三個同型的錯（前兩個是 `class="notice"` 沒有定義、
> 漏載兩支 `<script src>`）。**根因都是「用了在這個情境下不會生效的東西」，
> 而畫面上不會有任何錯誤訊息。**
...
### ⚠⚠ 新工具「文件拉正」整頁的 JS 是死的 —— 而每一關都是綠的

使用者回報：拉檔案進去沒反應，**點選檔案選完也沒反應**。

那支模板的 `{% block scripts %}` 裡**一行 `<script src>` 都沒有**。
`file_upload.js` / `job_progress.js` 不是全站載入的（`base.html` 只無條件載七支
共用的），每支工具自己載 —— 於是 `new FileUpload(...)` 在行內腳本的第一行丟
`ReferenceError`，**整段腳本停在那裡**：上傳沒接線、選項面板不會出現、
作業進度也不會動。

補上之後**同一支模板還有第二個同型的錯**：作業進度區寫成一個空的
`<div id="dsJob"></div>`，而 `JobProgress` 靠裡面的 `.job-bar` / `.job-status`
接線 → `addEventListener` 於 null 丟 `TypeError`，腳本改在那裡死掉。
（上傳區的同一個錯我上一版才修過，卻只修了上傳區。）

**為什麼畫面上看起來是好的**：上傳區是 `<label for>` 包 `<input type=file>`，
**純 HTML 就點得開檔案選擇器** —— 使用者以為功能在，選完檔案卻石沉大海，
而且畫面上沒有任何錯誤訊息（例外只進主控台）。

### 這一類以後測得出來了：每一頁都要在真的瀏覽器裡「活著」

**當時每一關都是綠的**，所以問題不在漏跑，在於**沒有任何一關會把頁面開起來跑
JS**：

| 關卡 | 為什麼看不到 |
|---|---|
| `pytest` 6,800+ 支 | 沒有一支會把工具頁開起來 |
| `node --check` 語法檢查 | 漏載 `<script src>` **語法完全合法** |
| 全站截圖逐張目視 | **畫面長得完全正常** |
| 端點測試 | 頁面回 200、API 全好 |

新增 `tests/test_pages_boot_in_a_browser.py`（測試計畫 §0.4）：把
**每一支工具頁 ＋ 首頁 / 我的作業 / 工作區**在無頭瀏覽器逐頁開一次，收
`Runtime.exceptionThrown` 與主控台 level=error（**含 CSP 違規**），任何一筆
就算失敗。**第一次跑就抓到上面那個 `JobProgress` 的錯**。

> 每頁用**自己的分頁**。第一版所有頁共用一個分頁，前一頁的事件會漏到下一頁
> ——症狀正是本專案很熟的那個：**單跑全綠、合跑失敗，而且失敗的是哪一頁還會變**。

另外補一支靜態守門 `tests/test_template_script_deps.py`：行內腳本 `new` 了哪個
共用元件，那一頁就必須自己載對應的 `.js`（判準只釘**硬相依**，
`window.X && window.X(...)` 這種做過特徵偵測的不算 —— 第一版沒分，一口氣誤報 17 支）。

### ⚠ 側欄一次標了兩支工具

開啟「註解平面化」時，「註解整理」也被標成使用中，而且自動捲動停在錯的那一支。
比對寫成 `path.startsWith(href)`，而 `/tools/pdf-annotations-flatten/` 的前綴
剛好就是 `/tools/pdf-annotations`（「註解清除」同一組）。判準改成
**完全相同、或下一個字元是 `/`**（子頁面仍然標得到父項）。

### ⚠ 說明框沒有樣式：`class="notice"` 全站沒有人定義

「文件拉正」的說明用了 `class="notice"`，而 `platform.css` **從來沒有這個類別**
——那段字就變成沒有樣式的純文字。收成共用的 `.info-box`
（「註解平面化」原本在自己的 `<style>` 裡寫了一份 `.af-info`，一併併過去，
不留兩份）。

### 用詞：「在線」→「線上」

使用者指出「在線」是大陸用法。使用者清單的「N 人在線」「在線」標記、
語系檔、README、介紹站、測試計畫全部換成「線上」，並加進用詞守門。

> **判準要排掉更長的正確詞**：「在線上」是正確的中文，只比對子字串的話
> 改對了反而會紅（實測當場踩到）。
...
客戶回報的情形是：伺服器本機（8765 或反向代理的 80）一切正常，**只有從遠端
電腦**拉檔案進去會出現「分析失敗：CSRF token 遺失或不正確」。網站看得到，
只有上傳會壞。
...
客戶回報文件翻譯只翻到第六頁、之後就產生不出來，但同時提到整體字數與原始
檔案接近。拿他們的檔案在同一個模型上實跑：
...
- 存檔改成**先寫暫存檔再 `os.replace`**：直接覆寫時寫到一半斷電會留下
  ...
  這樣沒了，而且沒有任何錯誤訊息。
- 管理頁的輸入框與下拉改掛全站既有的 `.field`（回報指出原文、譯文與下拉
  都沒有套到樣式）—— **不要各自寫一套**，那正是 SSO 設定頁長歪的原因。
...
- **登入頁的「認證領域」英文標籤太長**（回報指出帳號欄位的名稱太長）：
...
### 修正：帳號卡片的大頭像是一塊空白（帳號左邊留了一塊空白，看不出是大頭照）
...
### 修正：顏色的修法被自己的保險擋掉了（實測回報有些字的顏色沒有出來）
...
被問到測試清單有沒有用瀏覽器看過所有頁面的截圖 —— 誠實的答案是
...
回報指出這頁跑很久、預覽一直沒出來。正式機日誌顯示**每個預覽請求
...
客戶的網路是封閉的，連不到 GitHub 與 PyPI。補上兩條路，
**兩條都實機驗證過**：
...
客戶的觀察是 Git 裝完之後 Windows 要重開一次 —— 這個觀察正確，但根因是**我們不該
...
客戶需要在 SAML 認證失敗時查得到原因。它其實一直記在**稽核記錄**
...
被問到是不是一次只能選一個章蓋。功能其實早就有（「頁面個別用印」模式
...
- **結果訊息說明書籤在哪裡看**。回報指出看不到目錄，實際上書籤有做出來，
...
### 新增 — AD / LDAP 管理一輪（多 DC 容錯、主要群組、帳號狀態、批次停用、線上 session）

客戶指出 AD 帳號管理還有改善空間。盤點後分五項處理，每一項都是「企業
環境一定會遇到、但目前答不出來」的問題。
...
**線上 session：現在有誰登入著（schema v16）**

- `sessions` 原本只有建立時間，一個「記住我」的 session 可以躺 30 天 —— 看不出
  誰正在使用。新增 `last_seen_at`，使用者清單顯示線上人數與逐列標記。
...
- 回報指出 SSO 頁看起來沒套到樣式。用真實瀏覽器截圖比對才看出差在哪：輸入框本身
...
- 回報指出非 PDF 的檔案類型都沒有縮圖。原本的程式碼直接放棄這些格式 —— 理由沒錯（它們確實沒有便宜的「畫第一頁」方法，一定要經過 Office 引擎），但結論不對：畫面上就是一片空白，而使用者存進工作區的東西**大部分**就是轉出來的 Office 檔。
...
- 回報指出一離開該頁翻譯就停了，作業清單裡也看不到。原因是這個工具**從來沒有走過作業系統** —— 翻譯是瀏覽器自己驅動的：前端開 4 個工作緒逐句呼叫 API，結果存在那個分頁的記憶體裡。所以關掉分頁等於翻譯停止，而「我的作業」自然也看不到（根本沒有建立作業）。幾萬句的文件要人一直開著頁面等，不合理。
...
- 被問到帳號是不是缺了欄位，否則通知不知道要寄給誰 —— 確實缺：使用者資料表**從來沒有信箱欄位**。原本每個人都得自己到「我的作業 → 通知設定」手動填一次。
...
### 修正 — 個資限用章 / 插入日期沒有旋轉控制（客戶回報點了隨機旋轉沒有反應）
...
### 改善 — pdf-stamp 蓋章採「色彩增值」混合，蓋線上條 / 文字上更自然
...
之前 LLM 模式 hint 每條都長到三、四行，回報指出太冗長讀不下去。三條改成各 1-2 句話，重點直接 bold：
