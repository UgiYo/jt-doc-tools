...
使用者：「截圖 要放文件進去啊」「很多日文截圖都沒開範例檔」。根因不是忘了放：
...
使用者：「台灣專用功能 不需在 英文 日文 pages 截圖」。那幾支在英文 / 日文
介面下本來就是反灰的，介紹站還放它們的截圖只會讓人以為能用。
...
使用者：「還要可以選 修正完成後 的 size 是多少，可以用像素 也可以用 mm」
「預先自動抓 但 user 可以自己改」。
...
使用者：「正在重繪時 如果有其它動作也要觸發重繪，正在做的可以馬上停」。
...
v1.15.43 把線加粗了，使用者回報「還是沒有連線」。根因跟粗細無關：
...
使用者連續回報「排版不好看」「下拉清單沒套用本專案樣式」。逐一查下來全部是
